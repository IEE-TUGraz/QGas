...
<p>The local server connection for data transfer from the map to the local storage is managed via a python-based graphical user interface (GUI). Running Server.py in a Python interpreter like Visual Studio Code or PyCharm starts the GUI. Make sure to use the previously created environment to support all dependencies of the project.</p>
